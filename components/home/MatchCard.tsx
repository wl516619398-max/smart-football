...
export function MatchCard({ match, odds = fallbackOdds }: MatchCardProps) {
  ...
  const probabilities = [
    ...
    { label: "平", value: match.draw, className: "text-slate-200" },
  ...
  return <Link href={`/matches/${encodeURIComponent(match.id)}`} className="group block h-full"><Card className="h-full overflow-hidden border-slate-800/90 bg-[#111827] transition-all duration-200 group-hover:-translate-y-0.5 group-hover:border-blue-500/40 group-hover:shadow-glow">
    <div className="flex items-center justify-between gap-3 border-b border-slate-800/80 px-4 py-3"><div className="min-w-0"><p className="truncate text-[11px] font-medium text-blue-400">{match.league}</p><p className="mt-1 flex items-center gap-1.5 text-[11px] text-slate-500"><CalendarDays className="h-3 w-3" />{match.date} · {match.time}</p></div><span className={`shrink-0 rounded-full border px-2 py-1 text-[10px] font-medium ${level.className}`}>赔率价值 {level.label}</span></div>
    ...
    <div className="flex items-center justify-between gap-3 px-4 py-4"><div className="min-w-0"><p className="flex items-center gap-1.5 text-[11px] text-slate-500"><Sparkles className="h-3.5 w-3.5 text-amber-400" />AI信心 {match.aiScore}%</p><p className="mt-1 truncate text-sm font-semibold text-white">推荐方向：{match.prediction}</p></div><ArrowRight className="h-4 w-4 shrink-0 text-blue-400 transition-transform group-hover:translate-x-1" /></div>
  </Card></Link>;
}
