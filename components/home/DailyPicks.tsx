...
type DailyPick = { id: string; league: string; homeTeam: string; awayTeam: string; matchTime: string; homeWin: number; draw: number; awayWin: number; recommendation: string; confidence: number; value: number; valueLevel: string };
...
function statsFromProbabilities(homeWin: number, awayWin: number, confidence: number, isHome: boolean): PredictionTeamStats {
  ...
  return { attack: clamp(48 + ownProbability * 0.35 + confidence * 0.15), defense: clamp(52 + (100 - opponentProbability) * 0.25), form: clamp(44 + ownProbability * 0.3 + confidence * 0.2), homeAdvantage: isHome ? 90 : 50 };
}

function dateLabel(value: string) { return formatMatchDateTime(value).label; }
function valueLabel(value: number) { return value >= 0.05 ? "高价值" : value >= -0.05 ? "正常" : "低价值"; }
...
async function getDailyPicksMatches(): Promise<HomeMatchesResult> {
  ...
  if (supabase) {
    ...
    } catch { /* Use Mock data when the database is unavailable. */ }
  }
...
async function buildDailyPick(match: SyncedHomeMatch | FeaturedMatch, history: Awaited<ReturnType<typeof getPredictionHistory>>) {
  ...
  return { id: input.id, league: input.league, homeTeam: input.homeTeam, awayTeam: input.awayTeam, matchTime: input.matchTime, homeWin: prediction.homeWin, draw: prediction.draw, awayWin: prediction.awayWin, recommendation: prediction.recommendation[0] ?? historyRecord?.prediction ?? "双方数据接近", confidence: historyRecord?.confidence ?? prediction.confidence, value: bestValue, valueLevel: valueLabel(bestValue) } satisfies DailyPick;
}
...
export function DailyPicksLoading() { return <section className="mx-auto max-w-7xl px-4 pb-14 sm:px-6 lg:px-8"><div className="h-8 w-48 animate-pulse rounded bg-[#111827]" /><div className="mt-5 grid gap-5 lg:grid-cols-3">{[0, 1, 2].map((item) => <LoadingCard key={item} className="h-72" />)}</div></section>; }
