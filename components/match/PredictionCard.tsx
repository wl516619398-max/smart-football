...
export function PredictionCard({ match, prediction }: PredictionCardProps) {
  ...
  return <Card className="border-blue-500/30 bg-gradient-to-br from-blue-950/70 via-[#111827] to-[#111827] shadow-xl shadow-blue-950/20">
    ...
    <CardContent className="space-y-6">
      ...
      <div className="grid gap-3 border-t border-white/10 pt-4 sm:grid-cols-2"><div className="rounded-xl border border-emerald-500/15 bg-emerald-500/5 p-3"><p className="flex items-center gap-2 text-xs text-slate-500"><Activity className="h-4 w-4 text-emerald-400" />AI判断可信度</p><p className="mt-2 text-xl font-semibold text-emerald-300">{prediction.athenaScore}%</p><p className="mt-2 text-xs leading-5 text-slate-500">根据球队实力、近期表现和攻防数据综合计算。</p></div><div className="rounded-xl border border-amber-500/15 bg-amber-500/5 p-3"><p className="flex items-center gap-2 text-xs text-slate-500"><ShieldAlert className="h-4 w-4 text-amber-400" />分析可靠度</p><p className="mt-2 text-xl font-semibold text-amber-200">{prediction.riskLevel}</p><p className="mt-2 text-xs leading-5 text-slate-500">数据越完整，分析参考价值越高。</p></div></div>
