...
type FootballExpertAnalysisCardProps = {
  analysis: FootballAnalysisResult;
};

export function FootballExpertAnalysisCard({ analysis }: FootballExpertAnalysisCardProps) {
  return (
    <Card className="border-violet-500/20 bg-gradient-to-br from-[#16152f] via-[#111827] to-[#111827] shadow-xl shadow-violet-950/20">
      <CardHeader>
        <div className="flex items-center gap-2 text-xs font-medium text-violet-300"><BrainCircuit className="h-4 w-4" />ATHENA AI EXPERT ANALYSIS</div>
        <CardTitle className="mt-2 text-xl text-white">Athena AI专家分析</CardTitle>
        <p className="text-sm text-slate-400">基于球队状态、攻防指标与预测引擎结果生成</p>
      </CardHeader>
      <CardContent className="space-y-5">
        <div className="rounded-xl border border-violet-500/15 bg-violet-500/5 p-4">
          <p className="flex items-center gap-2 text-xs font-medium text-violet-200"><CheckCircle2 className="h-4 w-4" />AI分析</p>
          <ul className="mt-3 space-y-2 text-sm leading-6 text-slate-300">{analysis.analysis.map((item) => <li key={item} className="flex gap-2"><span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-violet-400" />{item}</li>)}</ul>
        </div>
        <div className="grid gap-3 md:grid-cols-3">
          <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-4"><p className="flex items-center gap-2 text-xs text-emerald-200"><CheckCircle2 className="h-4 w-4" />稳健</p><p className="mt-3 text-sm leading-6 text-slate-300">{analysis.recommendation.safe}</p></div>
