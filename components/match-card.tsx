...
export function MatchCard({ match, compact = false }: { match: FeaturedMatch; compact?: boolean }) {
  return <Card className={cn("group overflow-hidden transition-all duration-200 hover:-translate-y-0.5 hover:border-blue-500/40 hover:shadow-glow", compact ? "" : "h-full")}>
    ...
    <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3 px-4 py-5">
      <div className="flex min-w-0 flex-col items-center gap-2 text-center"><TeamBadge {...match.homeTeam} size={compact ? "sm" : "md"} /><span className="truncate text-sm font-medium text-slate-200">{match.homeTeam.name}</span></div>
      ...
      <div className="flex min-w-0 flex-col items-center gap-2 text-center"><TeamBadge {...match.awayTeam} size={compact ? "sm" : "md"} /><span className="truncate text-sm font-medium text-slate-200">{match.awayTeam.name}</span></div>
    </div>
